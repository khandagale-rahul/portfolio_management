module Upstox
  class SyncInstrumentHistoryJob < ApplicationJob
    ...
    def perform(unit: "day", interval: 1, days_back: 7)
      ...
      unless api_config
        log_error "[InstrumentHistory] No authorized Upstox API configuration found"
        return
      end

      if api_config.token_expired?
        log_error "[InstrumentHistory] Access token expired for API config #{api_config.id}"
        return
      end
      ...
      log_info "[InstrumentHistory] Found #{total_count} instruments to sync"
      ...
      instruments.find_each.with_index do |instrument, index|
        begin
          ...
          error_count += 1
          log_error "[InstrumentHistory] Error syncing #{instrument.symbol}: #{e.message}"
          log_error e.backtrace.first(5).join("\n")
        end
      end

      log_info "[InstrumentHistory] Sync completed at #{Time.current}"
      log_info "[InstrumentHistory] Results: success=#{success_count}, errors=#{error_count}, total=#{total_count}"
    end
  end
end
